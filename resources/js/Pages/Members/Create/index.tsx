...
const MembershipCreationForm = ({ auth, packages, lockers }: PageProps<{
    packages: MembershipPackage[],
    lockers: Locker[],
}>) => {
    const { data, setData, errors, post, processing, setError, clearErrors } = useForm({
        name: '',
        phone: '',
        occupation: '',
    ...
    useEffect(() => {
        const selectedPackage = packages.find(p => p.id === data.membership_package);
        if (selectedPackage) {
            ...
            const calculatedGrossAmount = admission + calculatedTotalMonthlyFees;

            let calculatedDiscount = 0;
            if (data.months === 3) {
                calculatedDiscount = Number(selectedPackage.discount_quarterly) || 0;
            } else if (data.months === 6) {
                calculatedDiscount = Number(selectedPackage.discount_half_yearly) || 0;
            } else if (data.months === 12) {
                calculatedDiscount = Number(selectedPackage.discount_yearly) || 0;
            }

            const tempNetPackageAmount = calculatedGrossAmount - calculatedDiscount;
            const validated_extra_discount = Math.min(data.extra_discount, tempNetPackageAmount);
            ...
            setNetPackageAmount(tempNetPackageAmount - validated_extra_discount);

            setData('extra_discount', validated_extra_discount);
        }

        if (data.locker_access) {
            const selectedLocker = lockers.find(locker => locker.id === data.locker_id);
            const lockerPrice = selectedLocker ? selectedLocker.price : 0;
            const validatedLockerDiscount = Math.min(data.locker_discount, lockerPrice);
    ...
    }, [data.membership_package, data.months, data.extra_discount, data.locker_access, data.locker_id, data.locker_discount, packages, lockers]);

    const calculateNetAmount = () => Number(data.months) === 0 ? data.net_amount : netPackageAmount + lockerCharge;
                                ...
                                <CardContent className="space-y-2">
                                    <div className="flex flex-wrap gap-2">
                                        <div className='grow'>
                                            ...
                                            <Select
                                                ...
                                                onValueChange={val => setData('membership_package', Number(val))}
                                            >
                                            ...
                                            <InputError message={errors.membership_package} className="mt-2" />
                                        </div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <div className='grow'>
                                            ...
                                            <InputError message={errors.payment_mode} className="mt-2" />
                                        </div>
                                        ...
                                        <div className='grow'>
                                            ...
                                            <Input
                                                id="bill_number"
                                                type="number"
                                                onChange={e => setData('bill_number', Number(e.target.value))}
                                                placeholder="Enter bill number"
                                            />
                                            <InputError message={errors.bill_number} className="mt-2" />
                                        </div>
                                            ...
                                            data.months > 0 ?
                                                ...
                                                <div className='grow'>
                                                    ...
                                                    <Input
                                                        ...
                                                        type="number"
                                                        onChange={e => setData('days_count', Number(e.target.value))}
                                                        required
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    ...
                                    <strong className="flex justify-between">
                                        ...
                                        {
                                            data.months > 0 ?
                                                <span className='cursor-pointer' onClick={() => setData('paid', calculateNetAmount())}>Rs {calculateNetAmount()}</span>
                                                :
                                                <input
                                                    id="paidAmount"
                                                    ...
                                                    onChange={e => setData('net_amount', Number(e.target.value))}
                                                />
                                        }
                                    </strong>
                                    <div className="flex justify-between">
                                    ...
                                    >
                                        {processing ? 'Processing...' : 'Confirm and Register'}
                                    </Button>
                                </CardContent>
                            </Card>
                        </div>
