...
export default function Transactions({ auth, data, activeTab }: PageProps<{ data: any; activeTab: string }>) {
    ...
    const switchTab = (tab: string) => {
        ...
        setCurrentTab(tab);
        router.get(route('transactions.index'), { tab }, {
    ...
    const TransactionFilter = () => {
        ...
        const handleSubmit = (e: FormEvent) => {
            ...
            router.visit(route('transactions.index') + '?' + updatedParams.toString());
        };
                    ...
                    <Tabs value={currentTab} onValueChange={switchTab}>
                        ...
                        {loading ? (
                            ...
                            <>
                                ...
                                {currentTab === 'registrations' && renderRegistrationTable(data)}
            ...
            <Table>
                ...
                <TableBody>
                    {data.data.map((registration: any) => {
                        const balance = Number(registration.net_amount) - Number(registration.paid_amount);
                        return (
                            <TableRow key={registration.id}>
                                <TableCell>{format(new Date(registration.payment_date), 'yyyy-MM-dd')}</TableCell>
                                <TableCell>{registration.member.name}</TableCell>
                                <TableCell>{registration.member.membership_package.package_name}</TableCell>
                                <TableCell>
                                    <Badge className='text-xs'>{registration.bill_number}</Badge>
                                </TableCell>
                                <TableCell>Rs {Number(registration.total_monthly_fees) + Number(registration.admission_fees) - Number(registration.package_discount)}</TableCell>
                                <TableCell>Rs {Number(registration.extra_discount)}</TableCell>
                                <TableCell>Rs {Number(registration.net_amount)}</TableCell>
                                <TableCell>Rs {Number(registration.paid_amount)}</TableCell>
                                <TableCell>
                                    {balance > 0 ? (
                                        <span className="text-red-500">Rs {balance} <sub><b>CR</b></sub></span>
                                    ) : balance < 0 ? (
                                        <span className="text-green-500">Rs {Math.abs(balance)} <sup><b>ADV</b></sup></span>
                                    ) : (
                                        <span>Rs 0</span>
                                    )}
                                </TableCell>
                                <TableCell>{registration.payment_mode}</TableCell>
                            </TableRow>
                        );
                    })}
                </TableBody>
            ...
            <Table>
                ...
                <TableBody>
                    {data.data.map((lockerPayment: any) => {
                        const credit = Number(lockerPayment.net_amount) - Number(lockerPayment.paid_amount); // Calculate credit
                        const duration = lockerPayment.total_months === 1 ? '1 Month' : `${lockerPayment.total_months} Months`; // Adjust duration grammar

                        return (
                            <TableRow key={lockerPayment.id}>
                                <TableCell>{new Date(lockerPayment.payment_date).toLocaleDateString()}</TableCell>
                                <TableCell>{lockerPayment.member?.name || 'Unknown'}</TableCell>
                                <TableCell>
                                    <Badge variant={'outline'} className='text-primary border-primary'>
                                        <strong>{lockerPayment.locker_number}</strong>
                                    </Badge>
                                </TableCell>
                                <TableCell>{duration}</TableCell> {/* Display grammatically correct duration */}
                                <TableCell>
                                    <Badge>{lockerPayment.bill_number}</Badge>
                                </TableCell>
                                <TableCell>Rs {Number(lockerPayment.locker_charge)}</TableCell>
                                <TableCell>Rs {Number(lockerPayment.locker_discount)}</TableCell>
                                <TableCell>Rs {Number(lockerPayment.net_amount)}</TableCell>
                                <TableCell>Rs {Number(lockerPayment.paid_amount)}</TableCell>
                                <TableCell>
                                    {credit > 0 ? (
                                        <span className="text-red-500">
                                            Rs {credit} <sub><b>CR</b></sub> {/* Credit */}
                                        </span>
                                    ) : credit < 0 ? (
                                        <span className="text-green-500">
                                            Rs {Math.abs(credit)} <sup><b>ADV</b></sup> {/* Advance */}
                                        </span>
                                    ) : (
                                        <span>Rs 0</span>
                                    )}
                                </TableCell>
                                <TableCell>{lockerPayment.payment_mode}</TableCell> {/* Expiry Date */}
                            </TableRow>
                        );
                    })}
                </TableBody>
            ...
            <Table>
                ...
                <TableBody>
                    {data.data.map((renewal: any) => {
                        const balance = Number(renewal.monthly_fees) * Number(renewal.total_months) - Number(renewal.extra_discount) - Number(renewal.package_discount) - Number(renewal.paid_amount);
                        const duration = renewal.total_months === 1 ? '1 Month' : `${renewal.total_months} Months`; // Adjust duration grammar

                        return (
                            <TableRow key={renewal.id}>
                                <TableCell>{format(new Date(renewal.payment_date).toLocaleDateString(), 'yyyy-MM-dd')}</TableCell>
                                <TableCell>{renewal.member.name}</TableCell>
                                <TableCell>{renewal.member.membership_package.package_name}</TableCell>
                                <TableCell>{duration}</TableCell> {/* Display grammatically correct duration */}
                                <TableCell>
                                    <Badge className='text-xs'>{renewal.bill_number}</Badge>
                                </TableCell>
                                <TableCell>Rs {Number(renewal.monthly_fees) * Number(renewal.total_months) - Number(renewal.package_discount)}</TableCell>
                                <TableCell>Rs {Number(renewal.extra_discount)}</TableCell>
                                <TableCell>Rs {Number(renewal.net_amount)}</TableCell>
                                <TableCell>Rs {Number(renewal.paid_amount)}</TableCell>
